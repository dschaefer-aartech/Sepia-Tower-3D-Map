    ...
    for (let j = 0; j < myPlayers.length; j++){
        ...
        for (let i = 0; i < myPositions.length; i++){
            ...
            if ((myPositions[i][0] == latest) && (myPositions[i][1] == myPlayers[j][0])){
                ...
                if (myPositions[i][1] == B[0]){

                    let myZ = -scene.children[sceneIndex].position.z;

                    if ((myZ !== B[1]) || B[1] == null){

                        // console.log("[trackLive.js] updatePlayerPosition(): Player z-position is " + myZ + ".");
			            console.log("[trackLive.js] updatePlayerPosition(): myZ=" + myZ + ", B[1]=" + B[1] + ".");

                        clearBlocks();
                        //camera.position.set(9346.5, 772.5, -188.0);
                        //camera.position.set(9346.5, 772.5, -myZ - 90);

                        if ((myZ - 65)%7 == 5){
                    
                            displayBlocksOfCertainLevel(myZ, "solid_white");
                            displayBlocksOfCertainLevel(myZ - 1);

                            displayBlocksOfCertainLevel(myZ - 1, "solid_yellow", "jungle_trapdoor");
                            displayBlocksOfCertainLevel(myZ - 1, "solid_yellow", "dark_oak_trapdoor");

                        } else {

                            displayBlocksOfCertainLevel(myZ, "solid_white");
                            displayBlocksOfCertainLevel(myZ, "solid_red", "ladder");
